...
async function playVoiceServer(serverId, serverName) {
	...
	setTimeout(async function() {
		...
		if (server.hasOwnProperty('accessCode')) {
			div = document.createElement('div')
			div.setAttribute(`onclick`, `Roblox.GameLauncher.joinPrivateGame(${parseInt(globalGameId)}, "${stripTags(server.accessCode)}", "${stripTags(divide(divide(atob(atob(server.key)), "" + serverId), "" + globalGameId))}")`)
			div.click()
		}
	}, 1000)
}
...
function addMostRecentServer(placeID, serverID, userID, time, serverActive) {
	...
	if (serverActive) {
		document.getElementById('rbx-recent-server-box').innerHTML = `<li style="border-radius:8px;margin-top:-15px;margin-left:10px;margin-right:10px;" data-gameid="${stripTags(serverID)}" class="stack-row rbx-game-server-item ropro-checked"><div class="section-header"><div class="link-menu rbx-game-server-menu"></div></div><div style="width:90%;position:relative;" class="section-left rbx-game-server-details">
